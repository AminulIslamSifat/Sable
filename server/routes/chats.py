...
from engine.config import MEMORY_PATH as _MEMORY_PATH, PROTECTED_PATH as _PROTECTED_PATH
from engine.config import MEMORY_SEARCH_SETTINGS_PATH as _MEMORY_SEARCH_SETTINGS
...
from server.database import get_messages, get_parent_id
from server.utils import is_deepseek_api_model, retry_async
...
async def consolidate_memory_scraper(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    # ... (rest of the consolidation logic, same as above)
    # I'll omit the duplicate logic for brevity - it's the same as consolidate_memory
    # In the actual implementation, you'd extract this to a shared function.
