...
function inlineMd(raw) {
  ...
  text = text.replace(
    /\u0000C(\d+)\u0000/g,
    (m, i) => `<code>${escHtml(codeStore[i])}</code>`,
  );
...
function renderBlockquote(lines) {
  ...
  if (calloutMatch) {
    ...
    return (
      `<div class="callout callout-${type}" style="--callout-color:${meta.color}">` +
...
function parseList(lines, i, baseIndent) {
  ...
  const itemsHtml = items
    ...
    .join("");
  return {
    html: `<${tag}${hasTasks ? ' class="contains-task-list"' : ""}>${itemsHtml}</${tag}>`,
    next: i,
  };
}
  ...
  while (i < lines.length) {
    ...
    if (fence) {
      ...
      if (/^mermaid$/i.test(lang)) {
        out.push(
          `<div class="mermaid-wrap"><pre class="mermaid">${escHtml(codeLines.join("\n"))}</pre></div>`,
        );
        ...
        out.push(
          `<pre><code${lang ? ` class="language-${escAttr(lang)}"` : ""}>${escHtml(codeLines.join("\n"))}</code></pre>`,
        );
      }
      continue;
    }
...
function renderMarkdown(raw) {
  if (!raw) return "";
  const text = escapeNonHtmlTags(
    normalizeMd(String(raw).replace(/\r\n/g, "\n")),
  );
...
function finishSkillCard(card, evt) {
  ...
  status.textContent =
    (evt.ok ? "done · " : "failed · ") + (evt.duration_ms ?? 0) + "ms";
...
if (diffClearBtn)
  diffClearBtn.addEventListener("click", () => {
    if (diffCardsEl) diffCardsEl.innerHTML = "";
  });
if (diffToggleBtn)
...
function handleFileEdit(evt, autoOpen) {
  ...
  revertBtn.title = evt.backup_path
    ? "Restore from backup"
    : "No backup available";
  ...
  if (!lines.length) {
    ...
    if (evt.truncated)
      frag.appendChild(
        diffLineEl("meta", "… preview truncated for performance"),
      );
...
function addHistoryMessage(message) {
  clearEmptyState();
  const events = Array.isArray(message.skill_events)
  ...
  if (message.role === "user") {
    ...
    const memMatch = displayContent.match(
      /^\[RELEVANT MEMORY CONTEXT\][\s\S]*?\n\n/,
    );
    if (memMatch) displayContent = displayContent.slice(memMatch[0].length);
    const tsMatch = displayContent.match(
  ...
  const msgDiv =
    message.role !== "user" && hasRoundText
      ? null
...
function addBotStreaming() {
  ...
  return {
    ...
    appendAnswer(text) {
      ...
      raw += text;
      if (text && text.trim() && !/\[(error|stopped|client error)\]/.test(text))
        sawNormalAnswer = true;
    ...
    replaceWithRateLimit(message, hours) {
      ...
      turn
        .querySelectorAll(".msg.bot, .skill-stack")
    ...
    showToolPending(evt) {
      ...
      const meta = {
        ...
        insert_file: {
          icon: "✏️",
          label: "Inserting into file",
          detail: attrs.path || "",
        },
        view_file: {
          icon: "👁️",
          label: "Reading file",
          detail: attrs.path || (attrs.full ? "full file" : ""),
        },
        execute_command: { icon: "⚡", label: "Running command", detail: "" },
        execute_background_command: {
          icon: "⚡",
          label: "Running background task",
          detail: "",
        },
      ...
      const detailHtml = info.progress
        ...
        : info.detail
          ? `<div class="tac-detail">${info.detail}</div>`
          : "";
      card.innerHTML =
        ...
        detailHtml +
        (info.progress
          ? `<div class="tac-progress-track"><div class="tac-progress-fill"></div></div>`
          : "") +
    ...
    finalize() {
      ...
      if (!turn.querySelector(".msg.bot")) {
        ...
        answerContent.textContent =
          "⚠ Empty response from upstream — check server terminal for WAF/auth details.";
      }
      ...
      turn.querySelectorAll(".msg.bot").forEach((botEl) => {
        ...
        regenBtn.addEventListener("click", () => {
          ...
          for (const u of allMsgs) {
            if (
              u.compareDocumentPosition(thisTurn) &
          ...
          const userText =
            prevUser.querySelector(".user-text")?.textContent || "";
          if (!userText) return;
      ...
      if (
        hasCommands &&
        !sawNormalAnswer &&
        !turn.querySelector(".retry-command-bar")
      ) {
        ...
        if (lastRound.length) {
          ...
          retryBtn.addEventListener("click", () =>
            retryLastCommand(lastRound, bar, retryBtn),
          );
...
function renderChats() {
  ...
  if (scraperChats.length > 0) {
    const header = document.createElement("div");
    header.className =
      "scraper-chats-header" + (scraperChatsCollapsed ? " collapsed" : "");
    header.innerHTML =
      '<span class="arrow">▼</span> 🌐 Scraper Chats (' +
      scraperChats.length +
      ")";
    header.onclick = () => {
    ...
    const body = document.createElement("div");
    body.className =
      "scraper-chats-body" + (scraperChatsCollapsed ? " collapsed" : "");
...
function populateThinkingModes(preferredModeId) {
  ...
  const modes =
    entry && entry.thinking_modes && entry.thinking_modes.length > 0
      ? entry.thinking_modes
      : [{ id: "thinking", label: "Thinking" }];

  selectedThinkingMode =
    preferredModeId && modes.some((m) => m.id === preferredModeId)
      ? preferredModeId
      : modes[0].id;
  ...
  try {
    localStorage.setItem(THINKING_MODE_KEY, selectedThinkingMode);
  } catch (err) {}
}
...
modelSelectEl.addEventListener("change", async () => {
  ...
  if (activeMeta?.provider) {
    ...
    const chatIsDs =
      activeMeta.provider === "deepseek" || activeMeta.provider === "scraping";
    if (newIsDs !== chatIsDs) {
  ...
  if (scraperMode) {
    try {
      ...
      if (data.status === "ok") {
        showToast(
          "Switched to " +
            (modelSelectEl.options[modelSelectEl.selectedIndex]?.textContent ||
              selectedModel),
          "success",
        );
        await createChat();
...
async function loadChats(mode) {
  try {
    ...
    const url = mode
      ? `/api/chats?mode=${encodeURIComponent(mode)}`
      : "/api/chats";
...
async function loadMessages(chatId) {
  try {
    const data = await fetch(`/api/chats/${chatId}/messages`).then((r) =>
      r.json(),
    );
    chatEl.innerHTML = "";
...
function lockModelDropdown(provider) {
  const allowed = provider
    ? modelList.filter((m) => {
        if (provider === "deepseek" || provider === "scraping")
          return m.api_backend === "deepseek";
  ...
  _consolidationPromise = fetch("/api/memory/consolidate", {
    ...
    .then(async (res) => {
      ...
      if (!res.ok) {
        ...
        showToast(
          `🧠 Consolidation failed (${res.status}): ${text.slice(0, 200)}`,
          "error",
        );
        return;
      }
      ...
      if (data.status === "ok") {
        ...
        showToast(
          `🧠 Consolidation failed: ${data.detail || "unknown error"}`,
          "error",
        );
      }
    })
...
async function createChat() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.status === "ok") {
          ...
          showToast(
            `🧠 Consolidation failed: ${data.detail || "unknown"}`,
            "error",
          );
        }
      } else {
  ...
  try {
    ...
    try {
      ...
      data = { error: `Server ${res.status}: ${text.slice(0, 300)}` };
    }
    if (!data.chat_id) {
...
async function sendMessage() {
  ...
  if (_consolidationPromise && scraperMode) {
    ...
    autoResize();
    showToast(
      "🧠 Message queued — waiting for memory consolidation...",
      "info",
    );
    return;
  }
  ...
  if (activeMeta?.provider) {
    if (scraperMode && activeMeta.provider !== "scraping") {
      showToast(
        "This chat is locked to " +
          activeMeta.provider +
          " — switch off scraper mode or start a new chat.",
        "error",
      );
      return;
    }
    if (!scraperMode && activeMeta.provider === "scraping") {
      showToast(
        "This is a scraping chat — enable scraper mode or start a new chat.",
        "error",
      );
      return;
    }
  ...
  chatEl
    .querySelectorAll(".file-edit-summary-card")
    .forEach((el) => el.remove());
  ...
  try {
    ...
    const { gotAnswer, gotDone, gotError } = await consumeChatStream(
      res,
      ui,
      userMsgDiv,
    );
    ...
    const emptyResponse =
      (!gotAnswer && !gotError && !gotDone) ||
      (gotDone && !gotAnswer && !gotError);
    if (emptyResponse) {
      const msg = gotDone
        ? "Response finished with no content"
    ...
    if (meta && !meta.provider) {
      const prov = scraperMode
        ? "scraping"
        : modelList.find((m) => m.id === selectedModel)?.api_backend ===
            "deepseek"
          ? "deepseek"
          : "qwen";
...
function renderMemory() {
  ...
  CATEGORIES.forEach((cat) => {
    ...
    btn.className =
      "icon-btn mem-tab-btn" + (cat === _activeCat ? " active" : "");
  ...
  entries.forEach((entry, idx) => {
    ...
    if (entry.expires_at) {
      ...
      badge.textContent =
        "⏳ " + String(entry.expires_at).replace("T", " ").slice(0, 16);
...
async function loadMemory() {
  try {
    ...
    if (raw && typeof raw === "object" && !Array.isArray(raw)) {
      ...
      _memoryCache = {
        semantic: Array.isArray(raw) ? raw : [],
        episodic: [],
        procedural: [],
        ephemeral: [],
      };
...
async function loadProtected() {
  try {
    ...
    console.error("loadProtected failed", e);
  }
}
...
function buildThresholdEditor(models, customThresholds) {
  ...
  header.style.cssText =
    "font-size:11px;margin:0 0 2px;text-transform:uppercase;letter-spacing:0.5px;";
  ...
  (models || []).forEach((m) => {
    ...
    label.style.cssText =
      "font-size:12px;flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;";
...
async function loadMemorySearchSettings() {
  ...
  try {
    ...
    document.getElementById("msMaxChars").value =
      data.max_prompt_chars || 20000;
    ...
    console.error("loadMemorySearchSettings failed", e);
  }
}
...
msSaveBtn.addEventListener("click", async () => {
  try {
    ...
    msThresholdEditor.querySelectorAll("input[data-model]").forEach((inp) => {
      if (inp.value.trim() !== "")
        modelThresholds[inp.dataset.model] = parseFloat(inp.value);
    });
    const res = await fetch("/api/settings/memory-search", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        ...
        max_prompt_chars:
          parseInt(document.getElementById("msMaxChars").value) || 20000,
...
function showSkillDetail(sk) {
  ...
  instrEl.innerHTML = sk.instruction
    ? renderMarkdown(sk.instruction)
    : "<em>No instruction file.</em>";
...
skillDetailOverlay.addEventListener("click", (e) => {
  if (e.target === skillDetailOverlay)
    skillDetailOverlay.classList.remove("show");
});

document
  .querySelector('[data-tab="skills"]')
  .addEventListener("click", loadSkills);
document
  .querySelector('[data-tab="account"]')
  .addEventListener("click", loadAccountProfiles);
...
function connectLogs() {
  ...
  logSource = new EventSource(
    "/api/logs?token=" + encodeURIComponent(getToken() || ""),
  );
...
const refreshDeepseekTokenBtn = document.getElementById(
  "refreshDeepseekTokenBtn",
);
...
if (refreshDeepseekTokenBtn) {
  ...
  refreshDeepseekTokenBtn.addEventListener("click", async () => {
    ...
    setDsStatus(
      "Refreshing DeepSeek token from browser profile…",
      "var(--text-dim)",
    );
    try {
      const res = await fetch("/api/settings/deepseek/refresh-token", {
        method: "POST",
      });
      const data = await res.json().catch(() => ({}));
      if (res.ok) {
        const preview = data.token_preview
          ? " (" + data.token_preview + ")"
          : "";
        setDsStatus(
          "✅ Token refreshed successfully" + preview,
          "var(--success, #3daa5c)",
        );
        ...
        const msg =
          data.detail || data.error || "DeepSeek token refresh failed";
...
async function loadAccountProfiles() {
  ...
  accountProfileCards.innerHTML =
    '<p class="muted" style="font-size:12px;margin:0;">Loading accounts…</p>';
  try {
    ...
    if (!accounts.length) {
      accountProfileCards.innerHTML =
        '<p class="muted" style="font-size:12px;margin:0;">No account profiles found. Create dirs like <code>system/browser-data-acc1</code>, <code>system/browser-data-acc2</code>…</p>';
      return;
    }

    accountProfileCards.innerHTML = accounts
      .map((acc) => {
    ...
    accountProfileCards
      .querySelectorAll(".account-switch-btn")
      .forEach((btn) => {
        btn.addEventListener("click", async () => {
          ...
          try {
            ...
            if (res.ok) {
              ...
              showToast(
                "Switch failed: " + (data.detail || "unknown"),
                "error",
              );
              btn.disabled = false;
...
(function loadTheme() {
  ...
  if (saved) {
    applyTheme(saved);
    const match = themePicker.querySelector(
      '.theme-swatch[data-theme="' + saved + '"]',
    );
    if (match) {
      themePicker
        .querySelectorAll(".theme-swatch")
        .forEach((b) => b.classList.toggle("active", b === match));
    }
  }
})();
...
async function loadScraperSettings() {
  try {
    ...
    if (res.ok) {
      ...
      if (scraperMode) {
        setScraperStatus(
          "● Browser connected — " + (data.engine_label || "Scraper"),
          "ok",
        );
      }
    }
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (data.enabled) {
        ...
        if (pl && pl.status === "ok") {
          setScraperStatus(
            "● Browser launched — " + (data.engine_label || "Scraper"),
            "ok",
          );
          ...
          setScraperStatus(
            "● Scraper enabled — " + (data.engine_label || ""),
            "ok",
          );
        }
...
scraperEngineSelect.addEventListener("change", async () => {
  ...
  try {
    const res = await fetch("/api/settings/scraper", {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        engine_type: scraperEngineSelect.value,
        enabled: true,
        headless: false,
      }),
    });
    if (res.ok) {
      ...
      if (pl && pl.status === "ok") {
        setScraperStatus(
          "● Browser relaunched — " + (data.engine_label || ""),
          "ok",
        );
      } else if (pl && pl.status === "error") {
        setScraperStatus("✗ " + pl.message, "err");
      ...
      showToast(
        "Engine switched to " +
          (data.engine_label || scraperEngineSelect.value),
        "success",
      );
      await loadModels();
    }
...
async function loadBrowserSession() {
  ...
  try {
    ...
    if (!d.active) {
      card.innerHTML =
        '<p class="muted" style="font-size:12px;margin:0;">No active browser session.</p>';
      return;
    }
    const alive = d.alive;
    ...
    card.innerHTML =
      ...
      '<span>Engine</span><span style="color:var(--text);">' +
      (d.engine_type || "\u2014") +
      "</span>" +
      '<span>Chat ID</span><span style="color:var(--text);">' +
      (d.chat_id || "\u2014") +
      "</span>" +
      '<span>PID</span><span style="color:var(--text);">' +
      (d.chrome_pid || "\u2014") +
      "</span>" +
      '<span>CDP Port</span><span style="color:var(--text);">' +
      (d.cdp_port || "\u2014") +
      "</span>" +
      '<span>Headless</span><span style="color:var(--text);">' +
      (d.headless ? "Yes" : "No") +
      "</span>" +
      '<span>URL</span><span style="color:var(--text);word-break:break-all;font-size:11px;">' +
      (d.page_url || "\u2014") +
      "</span>" +
      "</div>";
  } catch {
    card.innerHTML =
      '<p class="muted" style="font-size:12px;margin:0;color:var(--danger);">Failed to fetch session info.</p>';
  }
}
...
async function killBrowserSession() {
  try {
    ...
    showToast(
      d.killed_pid
        ? "Killed PID " + d.killed_pid
        : "Session reset (no PID found)",
      "success",
    );
  } catch {
...
document
  .getElementById("refreshSessionBtn")
  ?.addEventListener("click", loadBrowserSession);
loadBrowserSession();
  ...
  try {
    ...
    for (const [key, p] of Object.entries(d.profiles)) {
      ...
      html +=
        ...
        '<button data-profile="' +
        key +
        '" class="backupProfileBtn" style="background:var(--panel);color:var(--accent);border:1px solid var(--accent);border-radius:6px;padding:5px 12px;font-size:11px;cursor:pointer;font-weight:600;">📦 Backup</button>' +
        '<button data-profile="' +
        key +
        '" class="restoreProfileBtn" style="background:var(--accent);color:#fff;border:none;border-radius:6px;padding:5px 14px;font-size:11px;cursor:pointer;font-weight:600;' +
        (p.has_backup ? "" : "opacity:0.4;pointer-events:none;") +
        '">♻ Restore</button>' +
        "</span>" +
        "</div>" +
        '<div style="display:grid;grid-template-columns:auto 1fr;gap:2px 12px;font-size:12px;color:var(--text-dim);">' +
        '<span>Data</span><span style="color:var(--text);">' +
        p.data_dir +
        " (" +
        p.size_mb +
        " MB)</span>" +
        '<span>Backup</span><span style="color:var(--text);">' +
        bakDot +
        " " +
        (p.has_backup ? p.backup_size_mb + " MB" : "No backup found") +
        "</span>" +
        "</div>" +
        "</div>";
    }
    ...
    container.querySelectorAll(".restoreProfileBtn").forEach((btn) => {
      btn.addEventListener("click", () =>
        restoreBrowserProfile(btn.dataset.profile, btn),
      );
    });
    container.querySelectorAll(".backupProfileBtn").forEach((btn) => {
      btn.addEventListener("click", () =>
        createBrowserBackup(btn.dataset.profile, btn),
      );
    });
  } catch {
    container.innerHTML =
      '<p class="muted" style="font-size:12px;margin:0;color:var(--danger);">Failed to load profiles.</p>';
  }
}

async function restoreBrowserProfile(profile, btn) {
  const label =
    { api: "API (ChatService)", scraper: "Scraper", automation: "Automation" }[
      profile
    ] || profile;
  if (
    !confirm(
      "Restore " +
        label +
        " browser data from backup?\n\nThis DELETES the current profile and replaces it with the .bak snapshot. Make sure the browser is stopped.",
    )
  )
    return;
  btn.disabled = true;
  ...
  try {
    ...
    if (res.ok) {
      showToast(
        "♻ " + label + " profile restored from " + d.restored_from,
        "success",
      );
    } else {
...
document
  .getElementById("refreshProfilesBtn")
  ?.addEventListener("click", loadBrowserProfiles);
loadBrowserProfiles();
