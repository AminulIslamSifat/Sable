...
def benchmark_live(model_name: str, top_k: int = 5) -> dict | None:
    ...
    # Batch in chunks to avoid fastembed inhomogeneous-shape bug with variable-length inputs
    _BATCH = 16
    vec_chunks: list[np.ndarray] = []
    for b_start in range(0, len(memories), _BATCH):
        chunk = memories[b_start : b_start + _BATCH]
        vec_chunks.append(np.array(list(model.embed(chunk)), dtype="float32"))
    mem_vecs = np.vstack(vec_chunks)
...
def save_live_results(results: list[dict]) -> None:
    lines = []
    lines.append("=" * 90)
