...
async def lifespan(app: FastAPI) -> Generator[None, None, None]:
    init_db()
    # Restore persisted memory-search settings (model + custom threshold)
    if _MEMORY_SEARCH_SETTINGS.exists():
        try:
            ...
            if _ms.get("threshold") is not None:
                _s.set_threshold(float(_ms["threshold"]))
        except Exception:
            pass
...
async def get_memory_search_settings() -> dict[str, Any]:
    ...
    return {
        ...
        "custom_threshold": cfg.get("threshold"),
...
async def update_memory_search_settings(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if threshold is not None:
        if threshold == "auto" or threshold == "":
            cfg.pop("threshold", None)
            get_searcher().set_threshold(None)
        else:
            cfg["threshold"] = float(threshold)
            get_searcher().set_threshold(float(threshold))
